...
function drawRoute(map, instructionsData) {
  if (!routeData) {
    return;
  }
...
const mapRoutes = (cord) => {
  const map = new maplibregl.Map({
    center: [77.402201, 28.49639],
    zoom: 13,
    container: "map",
  });
  ...
  map.on("load", function () {
    ...
    drawRoute(map, instructionsData);
  });
};
